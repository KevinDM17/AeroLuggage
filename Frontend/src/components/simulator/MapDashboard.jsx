...
export default function MapDashboard({
  ...
  // Defaults en 0: si no hay sim corriendo, no inventamos números.
  const {
    bagsInTransit = 0,
    bagsDelivered = 0,
    bagsUnassigned = 0,
...
function Kpi({ icon: Icon, label, value, tone = "info" }) {
  ...
  return (
    <div className="bg-surface-1/70 backdrop-blur border border-slate-800 rounded-xl px-3 sm:px-4 py-2.5 flex items-center gap-3 min-w-0">
      <div className={`shrink-0 ${valueClass}`}>
        <Icon className="w-5 h-5" />
      </div>
      <div className="min-w-0 flex-1">
        {/* leading-tight + break-words: labels largos se parten en 2 líneas en vez de truncarse */}
        <div className="text-[10px] sm:text-xs text-slate-400 font-medium uppercase tracking-wider leading-tight break-words">
          {label}
        </div>
